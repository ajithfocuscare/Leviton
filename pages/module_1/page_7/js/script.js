...
var jsonSRC = 'pages/module_1/page_7/data/data.json?v='
...
function withAudioSync() {
    ...
    //tweenTimeline.add(animateDisplayNone($('.disableContainer')).play(), 16)
    ...
    if (correctAnswer == selectedAnswer) {
        ...
        if (noOfAttempted < _pageData.noOfAttempts) {
            ...
            setTimeout(function () {
                ...
                for (var m = 0; m < selectedAnswer.length; m++) {
                    if (correctAnswer[m] == 1)
                        $('#f_mc_optionContainer').find('.f_mc_optionBtn').eq(m).find('.f_mc_mark').removeClass('tick cross').addClass('tick')
                }
